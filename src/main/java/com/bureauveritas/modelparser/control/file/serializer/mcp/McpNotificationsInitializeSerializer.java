...
public class McpNotificationsInitializeSerializer extends AbstractMcpSerializer {

    private final String pathWithQuery;
    private final boolean isHttpStreaming;
    private final boolean withSessionId;

    public McpNotificationsInitializeSerializer(String pathWithQuery, String operationName, McpServerType serverType,
    ...
    public HttpRequest serializeRequest(String operationName, boolean includeOptionalParameters) {
        HttpRequest request = HttpRequest.httpRequest()
            ...
            .withBody(JSON_MAPPER.writeValueAsString(
                McpRequestBuilder.toolRequest(null, McpRequestBuilder.NOTIFICATIONS_INITIALIZED).build()
            ));
        if (isHttpStreaming) {
